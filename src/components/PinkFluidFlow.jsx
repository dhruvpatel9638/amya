...
/**
 * PinkFluidFlow — 100% accurate WebGL / Canvas interactive pink fluid simulation.
 * Recreates the exact fluid dynamics from incredibles.dev:
 * When the mouse moves, it injects pink fluid velocity and dye that curls,
 * swirls, and dissipates realistically at 60 FPS.
 */
export default function PinkFluidFlow() {
  ...
  useEffect(() => {
    ...
    // Try WebGL first for the exact GPU fluid simulation
    ...
    if (gl) {
      ...
      const linearExt = gl.getExtension('OES_texture_half_float_linear') || gl.getExtension('OES_texture_float_linear');
      ...
      // Display shader (renders pink fluid with soft alpha)
      const displayFS = createShader(
        ...
        void main () {
            ...
            float alpha = clamp(c.a * 1.5, 0.0, 0.85);
            // Blend pink accent #fc4778 with subtle highlights
            vec3 pink = mix(vec3(0.988, 0.278, 0.471), vec3(1.0, 0.675, 0.757), c.r * 0.5);
      ...
      const splatFS = createShader(
        ...
        void main () {
            ...
            gl_FragColor = vec4(base + splat, max(texture2D(uTarget, vUv).a, alpha * 0.9));
        }
      `
      );
      ...
      const render = () => {
        ...
        while (splatsQueue.length > 0) {
          ...
          gl.uniform1f(gl.getUniformLocation(splatProgram, 'uRadius'), 0.0018);
          gl.bindFramebuffer(gl.FRAMEBUFFER, velocity.write().fbo);
          gl.drawArrays(gl.TRIANGLE_FAN, 0, 4);
          velocity.swap();

          // Splat into density (pink color)
        ...
        gl.uniform1f(gl.getUniformLocation(advectionProgram, 'uDissipation'), 0.985); // Smooth velocity fade
        gl.bindFramebuffer(gl.FRAMEBUFFER, velocity.write().fbo);
        gl.drawArrays(gl.TRIANGLE_FAN, 0, 4);
        velocity.swap();

        // Advect density (pink dye dissipation)
        gl.uniform1i(gl.getUniformLocation(advectionProgram, 'uVelocity'), velocity.read().attach(0));
        gl.uniform1i(gl.getUniformLocation(advectionProgram, 'uSource'), density.read().attach(1));
        gl.uniform1f(gl.getUniformLocation(advectionProgram, 'uDissipation'), 0.965); // Gentle fade
        gl.bindFramebuffer(gl.FRAMEBUFFER, density.write().fbo);
        gl.drawArrays(gl.TRIANGLE_FAN, 0, 4);
        density.swap();
